import{s as b,C as a}from"./assets/sprite-ed217468.js";import{a as L}from"./assets/vendor-f728e3bf.js";const v="cartItems",O=document.querySelector("main");O.addEventListener("click",R);function R(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const i=s.dataset.id,o=s.querySelector("#product__image").src,r=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,l=s.querySelector("#product_size").textContent,u=s.querySelector("#product__price").textContent,p=s.querySelector(".card-icon-check"),m=s.querySelector(".card-icon-cart");if(!localStorage.getItem(v))M(v,[{id:i,img:o,imgDsc:r,title:c,category:n,price:u,size:l,quantity:1}]),m.style.display="none",p.style.display="block",w();else{const g=f(v),k=g.findIndex(x=>x.id===s.dataset.id);k!==-1?(g.splice(k,1),m.style.display="block",p.style.display="none"):(g.push({id:i,img:o,imgDsc:r,title:c,category:n,price:u,size:l,quantity:1}),m.style.display="none",p.style.display="block",console.log("ф-ція зміни картинки додати")),M(v,g),w()}}const M=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},f=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function w(){const t=document.querySelector(".header__js_span");f(v)?t.textContent=`CART (${f(v).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",w);function j({_id:t,name:e,img:s,desc:i,category:o,price:r,size:c,popularity:n,is10PercentOff:l}){let u=[],p=-1;return f("cartItems")&&(u=f("cartItems"),p=u.findIndex(m=>m.id===t)),`<li data-id="${t}" class="product__list__card product_item">
      ...
      <li data-id="${i}" class="discount-card product_item">
       ...
       </li>`).join("")}const lt=document.querySelector("#productsList"),d=document.querySelector(".js-modal-product-card"),_=document.querySelector(".js-backdrop"),H=document.querySelector(".loader-container");lt.addEventListener("click",dt);async function dt(t){if(t.target===t.currentTarget)return;const e=t.target.closest(".product__list__card");if(e===null)return;const s=e.dataset.id;if(t.target.closest(".add-to-cart"))return;H.classList.remove("visually-hidden");const i=await ut(s);d.innerHTML="";const{category:o,desc:r,img:c,name:n,popularity:l,price:u,size:p}=i,m=pt(o,r,c,n,l,u,p);d.innerHTML=m,H.classList.add("visually-hidden"),d.classList.remove("visually-hidden");const g=document.querySelector(".js-btn-close-card");g.addEventListener("click",k),_.addEventListener("click",x),document.addEventListener("keydown",P);function k(){_.classList.add("visually-hidden"),d.classList.add("visually-hidden"),d.innerHTML="",g.removeEventListener("click",k)}function x(){_.classList.add("visually-hidden"),d.classList.add("visually-hidden"),d.innerHTML="",_.removeEventListener("click",x)}function P(q){q.preventDefault(),q.key==="Escape"&&(_.classList.add("visually-hidden"),d.classList.add("visually-hidden"),d.innerHTML="",document.removeEventListener("keydown",P))}}async function ut(t){try{d.classList.remove("visually-hidden"),_.classList.remove("visually-hidden");const e="https://food-boutique.b.goit.study/api",s="products",{data:i}=await L.get(`${e}/${s}/${t}`);return i}catch(e){console.error(e)}}function pt(t,e,s,i,o,r,c){return`<div class="product-container">
