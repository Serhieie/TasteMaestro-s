import{s as g,C as a,d as Z,e as G}from"./assets/sprite-9dc59fac.js";import{a as b}from"./assets/vendor-f728e3bf.js";window.addEventListener("load",M);const v="cartItems",K=document.querySelector("main");K.addEventListener("click",Y);function Y(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(e===null)return;const i=t.target.closest(".add-to-cart").dataset.id,r=s.dataset.id,o=s.querySelector("#product__image").src,l=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,d=s.querySelector("#product_size").textContent,u=s.querySelector("#product__price").textContent;if(s.querySelector(".card-icon-check"),s.querySelector(".card-icon-cart"),!localStorage.getItem(v))z(v,[{id:r,img:o,imgDsc:l,title:c,category:n,price:u,size:d,quantity:1}]),M(),E(i);else{const p=y(v),k=p.findIndex(L=>L.id===s.dataset.id);k!==-1?(p.splice(k,1),E(i)):(p.push({id:r,img:o,imgDsc:l,title:c,category:n,price:u,size:d,quantity:1}),E(i)),z(v,p),M()}}const z=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},y=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function M(){const t=document.querySelector(".header__js_span");y(v)?t.textContent=`CART (${y(v).length})`:t.textContent=`CART (${0})`}function E(t){document.querySelectorAll(`.add-to-cart[data-id="${t}"]`).forEach(s=>{const i=s.querySelector(".card-icon-cart"),r=s.querySelector(".card-icon-check");i.style.display==="block"?(i.style.display="none",r.style.display="block"):(i.style.display="block",r.style.display="none")})}function Q({_id:t,name:e,img:s,desc:i,category:r,price:o,size:l,popularity:c,is10PercentOff:n}){let d=[],u=-1;return y("cartItems")&&(d=y("cartItems"),u=d.findIndex(p=>p.id===t)),`<li data-id="${t}" class="product__list__card product_item js-card">
