import{s as f,C as a}from"./assets/sprite-1cb1ef5a.js";import{a as L}from"./assets/vendor-f728e3bf.js";const b="cartItems",O=document.querySelector("main");O.addEventListener("click",R);function R(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const i=s.dataset.id,o=s.querySelector("#product__image").src,r=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,l=s.querySelector("#product_size").textContent,u=s.querySelector("#product__price").textContent,p=s.querySelector(".card-icon-check"),h=s.querySelector(".card-icon-cart");if(!localStorage.getItem(b))M(b,[{id:i,img:o,imgDsc:r,title:c,category:n,price:u,size:l,quantity:1}]),h.style.display="none",p.style.display="block",x();else{const g=m(b),k=g.findIndex($=>$.id===s.dataset.id);k!==-1?(g.splice(k,1),h.style.display="block",p.style.display="none"):(g.push({id:i,img:o,imgDsc:r,title:c,category:n,price:u,size:l,quantity:1}),h.style.display="none",p.style.display="block",console.log("ф-ція зміни картинки додати")),M(b,g),x()}}const M=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},m=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function x(){const t=document.querySelector(".header__js_span");m(b)?t.textContent=`CART (${m(b).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",x);function j({_id:t,name:e,img:s,desc:i,category:o,price:r,size:c,popularity:n,is10PercentOff:l}){let u=[],p=-1;return m("cartItems")&&(u=m("cartItems"),p=u.findIndex(h=>h.id===t)),`<li data-id="${t}" class="product__list__card product_item">
      ...
      </li>`}let S=a.filters.totalHits;a.paginationContainer.addEventListener("click",W);async function W(t){const e=parseInt(t.target.textContent);if(t.target.closest(".pagination-btn")){if(t.target.classList.contains("back")){if(a.filters.page===1)return;a.filters.page>0&&(a.filters.page-=1)}else if(t.target.classList.contains("forward")){if(a.filters.page===S){a.forward.setAttribute("disabled",!0);return}else a.filters.page<S&&a.forward.removeAttribute("disabled");a.filters.page+=1}_()}if(t.target.classList.contains("pagi_item_span")){if(t.target.textContent==="..."||e===a.filters.page)return;a.filters.page=e,_(),F()}}function F(){return document.querySelectorAll(".pagi_item").forEach(e=>{e.classList.remove("isActive"),parseInt(e.textContent)===a.filters.page&&e.classList.add("isActive")}),a.filters.page<=1?a.back.setAttribute("disabled",!0):a.back.removeAttribute("disabled"),a.filters.page===S?a.forward.setAttribute("disabled",!0):a.forward.removeAttribute("disabled")}function w(t,e){const s=e?"is-active":"",i=a.filters.page>=10?"py":"";return`<li class="pagi_item ${s} pagination_item"><span class="pagi_item_span ${i}">${t}</span></li>`}function T(){return'<li class="pagi_item pagination_item ellipsis"><span class="pagi_item_span pagi_item_span_ellipsis">...</span></li>'}function J(t,e){const s=Math.ceil(t);let i;window.innerWidth<768?i=1:i=2;const o=document.querySelector(".pagination-list");if(o.innerHTML="",s>1){let r="",c=1,n=s;s>5&&(c=Math.max(1,e-i),n=Math.min(s,e+i),c===1?window.innerWidth<768?n=3:n=5:n===s&&(c=s-4)),c>=2&&(r+=w(1,e===1),r+=T());for(let l=c;l<=n;l+=1)r+=w(l,l===e);n<s&&(r+=T(),r+=w(s,s===e)),o.innerHTML=r}}function U(t){t.length<a.filters.limit?a.paginationContainer.classList.add("visually-hidden"):a.paginationContainer.classList.remove("visually-hidden")}const Z=document.getElementById("filterForm"),I=document.getElementById("keywordInput"),N=document.getElementById("categorySelect"),C=document.querySelector(".category-list"),y=document.getElementById("sortProducts"),E=document.querySelector(".sortProducts-list");document.getElementById("productsList");const A=document.querySelector(".product__list");let B=[];const _=async()=>{try{let t=`https://food-boutique.b.goit.study/api/products?page=${a.filters.page}&limit=${a.filters.limit}`;if(a.filters.keyword&&(t+=`&keyword=${a.filters.keyword}`),a.filters.category&&a.filters.category!=="Show all"&&(t+=`&category=${a.filters.category}`),a.filters.sort&&a.filters.sort!=="all")switch(a.filters.sort){case"alphabetical":t+="&byABC=true";break;case"reverse-alphabetical":t+="&byABC=false";break;case"cheap":t+="&byPrice=true";break;case"expensive":t+="&byPrice=false";break;case"popular":t+="&byPopularity=false";break;case"not-popular":t+="&byPopularity=true";break}const s=(await L.get(t)).data;a.filters.totalHits=s.totalPages,G(s.results),U(s.results),J(s.totalPages,a.filters.page)}catch(t){console.error("Error fetching products:",t)}},G=t=>{t.length===0?A.innerHTML='<div class="nothing-found-conteiner"><p class="nothing-found">Nothing was found for the selected <span class="nothing-found_filter"> filters...</span></p><p class="inf-nothing-found">Try adjusting your search parameters or browse our range by other criteria to find the perfect product for you. </p></div>':A.innerHTML=t.map(e=>j(e)).join("")},K=async()=>{try{B=[...(await L.get("https://food-boutique.b.goit.study/api/products/categories")).data,"Show all"],V(B)}catch(t){console.error("Error fetching categories:",t)}},V=t=>{const e=t.map(s=>{let i=s.replace(/_/g," ");return s==="Breads_&_Bakery"&&(i=i.replace(/&/g,"/")),`<li class="category-item" data-value="${s}">${i}</li>`}).join("");C.innerHTML=e},Y=t=>{switch(t){case"alphabetical":y.textContent="A to Z";break;case"reverse-alphabetical":y.textContent="Z to A";break;case"cheap":y.textContent="Cheap";break;case"expensive":y.textContent="Expensive";break;case"popular":y.textContent="Popular";break;case"not-popular":y.textContent="Not popular";break;case"all":y.textContent="Show all";break}},z=()=>{E.classList.remove("show")};y.addEventListener("click",()=>{E.classList.toggle("show")});const Q=t=>{N.textContent=t},X=t=>t?t.getAttribute("data-value"):null,D=()=>{C.classList.remove("show")};N.addEventListener("click",()=>{C.classList.toggle("show")});document.addEventListener("click",t=>{!t.target.matches("#sortProducts")&&!t.target.closest(".sortProducts-list")&&z()});document.addEventListener("click",t=>{!t.target.matches("#categorySelect")&&!t.target.closest(".category-list")&&D()});I.addEventListener("input",()=>{a.filters.keyword=I.value,a.filters.page=1,_()});Z.addEventListener("submit",async t=>{t.preventDefault(),a.filters.keyword=I.value,a.filters.page=1,_()});C.addEventListener("click",t=>{if(t.target.classList.contains("category-item")){const e=X(t.target);a.filters.category=e,a.filters.page=1,_(),Q(e),D()}});E.addEventListener("click",t=>{if(t.target.classList.contains("category-item")){const e=t.target.getAttribute("data-value");a.filters.sort=e,a.filters.page=1,_(),Y(e),z()}});K();_();let tt=5;const et=document.querySelector(".popular-js");st();async function st(){try{const t=await L.get(`https://food-boutique.b.goit.study/api/products/popular?limit=${tt}`);et.insertAdjacentHTML("beforeend",at(t))}catch{console.log("Упс! Щось пішло не так.")}}function at(t){return t.data.map(e=>j(e)).join("")}const it={discountCardList:document.querySelector(".discount-card-list")};rt();async function rt(){try{const t=await ot(),e=ct(t);it.discountCardList.innerHTML=nt(e)}catch(t){console.log(t)}}async function ot(){const t="https://food-boutique.b.goit.study/api",e="products/discount";try{const{data:s}=await L.get(`${t}/${e}`);return s}catch(s){console.log(s)}}function ct(t){const e=[...t],s=Math.floor(Math.random()*e.length),i=e[s];e.splice(s,1);const o=Math.floor(Math.random()*e.length),r=e[o];return[i,r]}function nt(t){let e=[],s=-1;return m("cartItems")&&(e=m("cartItems"),s=e.findIndex(i=>i.id===t._id)),console.log(m("cartItems")),t.map(({_id:i,name:o,img:r,category:c,price:n,size:l,popularity:u})=>`
