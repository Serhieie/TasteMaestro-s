import{C as a}from"./assets/footer-0002a36a.js";import{a as b}from"./assets/vendor-f728e3bf.js";const p="/TasteMaestro-s/assets/sprite-c044ac3a.svg",g="cartItems",T=document.querySelector("main");T.addEventListener("click",B);function B(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const r=s.dataset.id,o=s.querySelector("#product__image").src,i=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,h=s.querySelector("#product_size").textContent,_=s.querySelector("#product__price").textContent,d=s.querySelector(".card-icon-check"),f=s.querySelector(".card-icon-cart");if(!localStorage.getItem(g))L(g,[{id:r,img:o,imgDsc:i,title:c,category:n,price:_,size:h,quantity:1}]),f.style.display="none",d.style.display="block",w();else{const y=m(g),k=y.findIndex(A=>A.id===s.dataset.id);console.log(k),k!==-1?(y.splice(k,1),f.style.display="block",d.style.display="none"):(y.push({id:r,img:o,imgDsc:i,title:c,category:n,price:_,size:h,quantity:1}),f.style.display="none",d.style.display="block",console.log("ф-ція зміни картинки додати")),L(g,y),w()}}const L=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},m=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function w(){const t=document.querySelector(".header__js_span");m(g)?t.textContent=`CART (${m(g).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",w);function H({_id:t,name:e,img:s,desc:r,category:o,price:i,size:c,popularity:n,is10PercentOff:h}){let _=[],d=-1;return m("cartItems")&&(_=m("cartItems"),d=_.findIndex(f=>f.id===t)),`<li data-id="${t}" class="product__list__card product_item">
