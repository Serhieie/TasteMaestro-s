import{C as a}from"./assets/footer-f9b4350d.js";import{a as b}from"./assets/vendor-f728e3bf.js";const p="/TasteMaestro-s/assets/sprite-c044ac3a.svg",g="cartItems",B=document.querySelector("main");B.addEventListener("click",j);function j(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const r=s.dataset.id,i=s.querySelector("#product__image").src,o=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,m=s.querySelector("#product_size").textContent,f=s.querySelector("#product__price").textContent,d=s.querySelector(".card-icon-check"),_=s.querySelector(".card-icon-cart");if(!localStorage.getItem(g))L(g,[{id:r,img:i,imgDsc:o,title:c,category:n,price:f,size:m,quantity:1}]),_.style.display="none",d.style.display="block",w();else{const y=h(g),x=y.findIndex(T=>T.id===s.dataset.id);console.log(x),x!==-1?(y.splice(x,1),_.style.display="block",d.style.display="none"):(y.push({id:r,img:i,imgDsc:o,title:c,category:n,price:f,size:m,quantity:1}),_.style.display="none",d.style.display="block",console.log("ф-ція зміни картинки додати")),L(g,y),w()}}const L=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},h=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function w(){const t=document.querySelector(".header__js_span");h(g)?t.textContent=`CART (${h(g).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",w);function H({_id:t,name:e,img:s,desc:r,category:i,price:o,size:c,popularity:n,is10PercentOff:m}){let f=[],d=-1;return h("cartItems")&&(f=h("cartItems"),d=f.findIndex(_=>_.id===t)),`<li data-id="${t}" class="product__list__card product_item">
          ...
          <ul class="product__details">
            <li>
              <p class="product__description__text">
                Category:
                <span id="product_category_name" class="product__description__span"}>${i.replace(/_/g," ")}</span>
              </p>
            </li>
            <li>
              <p class="product__description__text">
        ...
        <div class="popular_description_container">
          ...
          <ul class="popular_details">
            ...
            <li class="popular_details_item">
              <p class="popular_details_category">
                Size: <span class="popular_details_text">${i}</span>
              </p>
            </li>
            <li class="popular_details_item">
              <p class="popular_details_category">
                Popularity: <span class="popular_details_text">${o}</span>
              </p>
            </li>
            </div>
          </ul>
  ...
  <li class="discount-card">
      ...
      </li>`).join("")}function st(t){const e=[...t],s=Math.floor(Math.random()*e.length),r=e[s];e.splice(s,1);const i=Math.floor(Math.random()*e.length),o=e[i];return[r,o]}
