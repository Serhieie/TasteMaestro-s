import{s as h,C as a}from"./assets/sprite-5b0f421c.js";import{a as x}from"./assets/vendor-f728e3bf.js";const v="cartItems",N=document.querySelector("main");N.addEventListener("click",O);function O(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const i=s.dataset.id,n=s.querySelector("#product__image").src,r=s.querySelector("#product__image").alt,o=s.querySelector("#product__title").textContent,c=s.querySelector("#product_category_name").textContent,l=s.querySelector("#product_size").textContent,g=s.querySelector("#product__price").textContent,d=s.querySelector(".card-icon-check"),y=s.querySelector(".card-icon-cart");if(!localStorage.getItem(v))M(v,[{id:i,img:n,imgDsc:r,title:o,category:c,price:g,size:l,quantity:1}]),y.style.display="none",d.style.display="block",$();else{const p=k(v),b=p.findIndex(L=>L.id===s.dataset.id);b!==-1?(p.splice(b,1),y.style.display="block",d.style.display="none"):(p.push({id:i,img:n,imgDsc:r,title:o,category:c,price:g,size:l,quantity:1}),y.style.display="none",d.style.display="block",console.log("ф-ція зміни картинки додати")),M(v,p),$()}}const M=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},k=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function $(){const t=document.querySelector(".header__js_span");k(v)?t.textContent=`CART (${k(v).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",$);function R({_id:t,name:e,img:s,desc:i,category:n,price:r,size:o,popularity:c,is10PercentOff:l}){let g=[],d=-1;return k("cartItems")&&(g=k("cartItems"),d=g.findIndex(y=>y.id===t)),`<li data-id="${t}" class="product__list__card product_item">
  ...
  <li class="discount-card">
      ...
      </li>`).join("")}function ct(t){const e=[...t],s=Math.floor(Math.random()*e.length),i=e[s];e.splice(s,1);const n=Math.floor(Math.random()*e.length),r=e[n];return[i,r]}const nt=document.querySelector("#productsList"),f=document.querySelector(".js-modal-product-card"),_=document.querySelector(".js-backdrop"),j=document.querySelector(".loader-container");nt.addEventListener("click",lt);f.classList.add("visually-hidden");async function lt(t){if(t.target===t.currentTarget)return;const e=t.target.closest(".product__list__card");if(e===null)return;const s=e.id;j.classList.remove("visually-hidden");const i=await dt(s),{category:n,desc:r,img:o,name:c,popularity:l,price:g,size:d}=i,y=pt(n,r,o,c,l,g,d);f.innerHTML=y,j.classList.add("visually-hidden"),f.classList.remove("visually-hidden");const p=document.querySelector(".js-btn-close-card");p.addEventListener("click",b),_.addEventListener("click",L),document.addEventListener("keydown",P);function b(){_.classList.add("visually-hidden"),f.classList.add("visually-hidden"),p.removeEventListener("click",b)}function L(){_.classList.add("visually-hidden"),f.classList.add("visually-hidden"),_.removeEventListener("click",L)}function P(q){q.preventDefault(),q.key==="Escape"&&(_.classList.add("visually-hidden"),f.classList.add("visually-hidden"),document.removeEventListener("keydown",P))}}async function dt(t){try{f.classList.remove("visually-hidden"),_.classList.remove("visually-hidden");const e="https://food-boutique.b.goit.study/api",s="products",{data:i}=await x.get(`${e}/${s}/${t}`);return i}catch(e){console.error(e)}}function pt(t,e,s,i,n,r,o){return`<div class="product-container">
