import{s as h,C as a}from"./assets/sprite-23d827d5.js";import{a as k}from"./assets/vendor-f728e3bf.js";const m="cartItems",D=document.querySelector("main");D.addEventListener("click",N);function N(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const i=s.dataset.id,o=s.querySelector("#product__image").src,r=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,l=s.querySelector("#product_size").textContent,g=s.querySelector("#product__price").textContent,y=s.querySelector(".card-icon-check"),_=s.querySelector(".card-icon-cart");if(!localStorage.getItem(m))E(m,[{id:i,img:o,imgDsc:r,title:c,category:n,price:g,size:l,quantity:1}]),_.style.display="none",y.style.display="block",C();else{const v=p(m),I=v.findIndex(z=>z.id===s.dataset.id);I!==-1?(v.splice(I,1),_.style.display="block",y.style.display="none"):(v.push({id:i,img:o,imgDsc:r,title:c,category:n,price:g,size:l,quantity:1}),_.style.display="none",y.style.display="block",console.log("ф-ція зміни картинки додати")),E(m,v),C()}}const E=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},p=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function C(){const t=document.querySelector(".header__js_span");p(m)?t.textContent=`CART (${p(m).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",C);function A({_id:t,name:e,img:s,desc:i,category:o,price:r,size:c,popularity:n,is10PercentOff:l}){let g=[],y=-1;return p("cartItems")&&(g=p("cartItems"),y=g.findIndex(_=>_.id===t)),`<li data-id="${t}" class="product__list__card product_item">
