import{s as u,C as a}from"./assets/sprite-74d0a6c3.js";import{a as $}from"./assets/vendor-f728e3bf.js";const b="cartItems",D=document.querySelector("main");D.addEventListener("click",O);function O(t){const e=t.target.closest(".add-to-cart"),s=t.target.closest(".product_item");if(!s||s.nodeName!=="LI")return;if(e===null){console.log("modal");return}const i=s.dataset.id,o=s.querySelector("#product__image").src,r=s.querySelector("#product__image").alt,c=s.querySelector("#product__title").textContent,n=s.querySelector("#product_category_name").textContent,l=s.querySelector("#product_size").textContent,p=s.querySelector("#product__price").textContent,d=s.querySelector(".card-icon-check"),_=s.querySelector(".card-icon-cart");if(!localStorage.getItem(b))M(b,[{id:i,img:o,imgDsc:r,title:c,category:n,price:p,size:l,quantity:1}]),_.style.display="none",d.style.display="block",w();else{const h=y(b),k=h.findIndex(L=>L.id===s.dataset.id);k!==-1?(h.splice(k,1),_.style.display="block",d.style.display="none"):(h.push({id:i,img:o,imgDsc:r,title:c,category:n,price:p,size:l,quantity:1}),_.style.display="none",d.style.display="block",console.log("ф-ція зміни картинки додати")),M(b,h),w()}}const M=(t,e)=>{try{const s=JSON.stringify(e);localStorage.setItem(t,s)}catch(s){console.error("Set state error: ",s.message)}},y=t=>{try{const e=localStorage.getItem(t);return e===null?void 0:JSON.parse(e)}catch(e){console.error("Get state error: ",e.message)}};function w(){const t=document.querySelector(".header__js_span");y(b)?t.textContent=`CART (${y(b).length})`:t.textContent=`CART (${0})`}window.addEventListener("load",w);function R({_id:t,name:e,img:s,desc:i,category:o,price:r,size:c,popularity:n,is10PercentOff:l}){let p=[],d=-1;return y("cartItems")&&(p=y("cartItems"),d=p.findIndex(_=>_.id===t)),`<li data-id="${t}" class="product__list__card product_item">
